...
def _derive_hints(pods_text: str, warn_summary: dict) -> list[str]:
    ...
    if ("ImagePullBackOff" in t) or ("ErrImagePull" in t):
        hints.append("Check image & registry creds; `describe pod` to see pull errors")
    if "Pending" in t:
        hints.append("Investigate scheduling: `describe pod` (FailedScheduling?)")
    if "NotReady" in t:
        hints.append("Node not ready: `describe node <node>` (conditions/events)")
    reasons = {r.lower() for r in (warn_summary.get("count_by_reason") or {}).keys()}
    if "failedscheduling" in reasons:
        hints.append("FailedScheduling ? check taints/resources")
    if "backoff" in reasons:
        hints.append("Container backoff ? inspect previous logs + probe timings")
    if "unhealthy" in reasons:
        hints.append("Probe failures ? fix port/path or increase initialDelay")
